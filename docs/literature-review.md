...
Individual evidence-reference digests bind content, not independently authenticated
producer origin. MRA also has signed manifests and authenticated lifecycle records;
these must not be confused with unsigned references or with an independently
attested collection process. Neither a digest nor an authorized signature proves
that the experiment was actually executed correctly or disclosed every relevant result.
...
([Necula, 1997](https://doi.org/10.1145/263699.263712)). For analyzer-extension
conservativity, MRA establishes the fixed-prerequisite record-level property that additions with neither
...
| Failed empirical attack | This attack failed under this data, power, and budget | Absence of leakage | A valid design may still supply a low floor; an invalid or underpowered operating point is a `screen`; neither determines the overall verdict |
...
| Rank exposure from auditor-only logits | Model memorization diagnostic | Recipient-realizable black-box leakage or a probability/advantage bound without a separately registered game and derivation | diagnostic `screen`, not a privacy floor |
| Watermark detector hit | Rejection of the registered no-watermark null for a declared key-associated detector at its calibrated threshold | Authorship, provider identity, or privacy | provenance `screen` only; cannot clear or block a privacy threat |
| Watermark detector miss | Failure to reject the registered null under the tested text length, entropy, transformations, and detector power | Human authorship or non-generation | provenance `screen`; does not determine the privacy verdict |
...
| Three distinct canary modes | The profile separates the designs; `LlmCanaryAnalyzer` validates registered canary-count evidence as a floor or screen, never a ceiling | Retain separate exposure diagnostics and registered randomized IN/OUT or extraction games; complete collection/replay workers before claiming end-to-end coverage of every design |
| Watermark null calibration and adaptive attack suite | The profile/linter declares the requirements; `LlmWatermarkAnalyzer` consumes calibration/detection counts as provenance screens, not a complete detector/attack worker | Implement replayable collection, scoring, quality, power, and attack-matrix evidence; never turn a watermark screen into privacy clearance |
...
<a id="strategic-incentives-review"></a>

## Strategic incentives: primary-source review

This section consolidates the targeted review dated below. Its original source
claims and transfer limits are retained; the 2026-09-07 offline consolidation is
not a new literature search or independent bibliographic re-verification. The
machine-readable [claim ledger](../formal/game-theory-claim-ledger-v1.json) retains
the source records. Current executable behavior is described in the
[strategic stress-test instructions](mathematical-foundations.md#strategic-stress-test-execution).
