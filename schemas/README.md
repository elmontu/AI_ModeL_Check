...
- `release-protocol-run-v1.1.json`, the current MRAP/1.0 lifecycle transcript contract with structural and authenticated verification profiles, signed event/artifact declarations, and append-only registry sequences
...
The assessment v1/v2, optimization v1, policy-bundle v1, signed-manifest v1, and structural `release-protocol-run-v1.json` files are retained for provenance and compatibility. They do not override the current contracts in `src/model_release_assurance/`.
